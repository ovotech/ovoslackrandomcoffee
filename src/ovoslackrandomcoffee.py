...
def get_previous_pairs(channel, testing, lookback_days=LOOKBACK_DAYS):
    ...
    if testing:
        texts = [
            ...
            if MAGICAL_TEXT in t["text"] and "@" in t["text"]
        ]
    else:
        texts = [
            t["text"]
            for t in conversation_history
            if MAGICAL_TEXT in t["text"] and "<@U" in t["text"]
        ]
    ...
    try:
        ...
        if testing:
            members = [
                f'@{u["name"]}'
                for u in users_list
                if u["id"] in member_ids and not u["is_bot"]
            ]
        else:
            members = [
                f'<@{u["id"]}>'
...
def ovoslackrandomcoffee(work_ids=None, testing=False):
...
if __name__ == "__main__":
    ovoslackrandomcoffee(testing=True)
